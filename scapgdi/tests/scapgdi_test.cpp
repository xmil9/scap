...
// Error class for scapgdi tests.
class ScapgdiTestError : public std::runtime_error {
public:
   explicit ScapgdiTestError(const std::string& msg) : std::runtime_error(msg) {
   }
};
...
PngInfo loadBGRAPngImage(std::string const& filepath,
   ...
   try {
      ...
      if (info.format() != PNG_FORMAT_BGRA)
         throw ScapgdiTestError("Unexpected PNG data format.");
      
      return info;
   } catch (pngutil::Error& ex) {
      throw ScapgdiTestError(ex.what());
   }
}
...
void runTests(std::string const& testDataDir) {
   ...
   if (totalCasesFailed == 0)
      std::cout << "scapgdi tests SUCCEEDED." << std::endl;
   else
      std::cout << "scapgdi tests FAILED. Failed test cases: "
