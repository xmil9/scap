...
// Error class for scapx tests.
class ScapXTestError : public std::runtime_error {
public:
   explicit ScapXTestError(const std::string& msg) : std::runtime_error(msg) {
   }
};
...
// image is RGBA.
PngInfo loadRGBAPngImage(std::string const& filepath,
   ...
   try {
      ...
      PngInfo info = loadPng(filepath, PNG_FORMAT_RGBA, allocatePixelBuffer);
      if (info.format() != PNG_FORMAT_RGBA)
         throw ScapXTestError("Unexpected PNG data format.");

      return info;
   } catch (pngutil::Error& ex) {
      throw ScapXTestError(ex.what());
   }
}
...
bool isBlankPngImage(std::string const& filepath) {
   ...
   loadRGBAPngImage(filepath, &pixelBuffer);
...
void runTests(std::string const& testDataDir) {
   ...
   if (totalCasesFailed == 0)
      std::cout << "scapx tests SUCCEEDED." << std::endl;
   else
      std::cout << "scapx tests FAILED. Failed test cases: "
